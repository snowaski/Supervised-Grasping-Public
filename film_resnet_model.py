...
class Model(object):
  ...
  def __call__(self,
    ...
    with self._model_variable_scope():
      ...
      img1 = conv2d_fixed_padding(
          inputs=inputs.RGB, filters=filters[0], kernel_size=self.kernel_size,
          strides=self.conv_stride, data_format=self.data_format,
          weight_decay=self.weight_decay)
      img1 = tf.identity(img1, 'initial_conv1')

      img2 = conv2d_fixed_padding(
          inputs=inputs.Feature_RGB, filters=filters[1], kernel_size=self.kernel_size,
          strides=self.conv_stride, data_format=self.data_format,
          weight_decay=self.weight_decay)
      img2 = tf.identity(img2, 'initial_conv2')

      img3 = conv2d_fixed_padding(
          inputs=inputs.Depth, filters=filters[2][0], kernel_size=self.kernel_size,
          strides=self.conv_stride, data_format=self.data_format,
          weight_decay=self.weight_decay)
      img3 = tf.identity(img3, 'initial_conv3')

      img4 = conv2d_fixed_padding(
          inputs=inputs.Depth, filters=filters[3], kernel_size=self.kernel_size,
          strides=self.conv_stride, data_format=self.data_format,
          weight_decay=self.weight_decay)
      ...
      if self.first_pool_size:
        img1 = tf.layers.max_pooling2d(
            inputs=img1, pool_size=self.first_pool_size,
            strides=self.first_pool_stride, padding='SAME',
            data_format=self.data_format)
        img1 = tf.identity(img1, 'initial_max_pool1')

        img2 = tf.layers.max_pooling2d(
            inputs=img2, pool_size=self.first_pool_size,
            strides=self.first_pool_stride, padding='SAME',
            data_format=self.data_format)
        img2 = tf.identity(img2, 'initial_max_pool2')

        img3 = tf.layers.max_pooling2d(
            inputs=img3, pool_size=self.first_pool_size,
            strides=self.first_pool_stride, padding='SAME',
            data_format=self.data_format)
        img3 = tf.identity(img3, 'initial_max_pool3')

        img4 = tf.layers.max_pooling2d(
            inputs=img4, pool_size=self.first_pool_size,
            strides=self.first_pool_stride, padding='SAME',
            data_format=self.data_format)
      ...
      for i, num_blocks in enumerate(self.block_sizes):
        ...
        img1 = block_layer(
            inputs=img1, filters=(filters[0] * (2**i)), bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], training=training,
            name='rgb_block_layer{}'.format(i + 1), data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)

        img2 = block_layer(
            inputs=img2, filters=(filters[1] * (2**i)), bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], training=training,
            name='feat_rgb_block_layer{}'.format(i + 1), data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)

        img3 = block_layer(
            inputs=img3, filters=(filters[2][i+1]), bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], training=training,
            name='depth_block_layer{}'.format(i + 1), data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)

        img4 = block_layer(
            inputs=img4, filters=(filters[3] * (2**i)), bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], training=training,
            name='feat_depth_block_layer{}'.format(i + 1), data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)
      ...
      inputs = block_layer(
            inputs=inputs, filters=64, bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=3,
            strides=self.block_strides[i], training=training,
            name='tower_block_layer1', data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)
      ...
      inputs = block_layer(
            ...
            block_fn=self.block_fn, blocks=3,
            strides=self.block_strides[i], training=training,
            name='tower_block_layer2', data_format=self.data_format,
            weight_decay=self.weight_decay,
            film_gamma_betas=None)

      inputs = conv2d_fixed_padding(
          inputs=inputs, filters=1, kernel_size=8,
          strides=self.conv_stride, data_format=self.data_format,
          weight_decay=self.weight_decay)
      ...
      assert(not type(inputs) is tuple), f'{inputs} is a tuple. Fix that'
